...
export function FaxPageViewer({
  ...
  useEffect(() => {
    if (activePageNumber !== null && activePageNumber !== currentPageIndex + 1) {
      setTransitioning(true);
      onPageChange(activePageNumber - 1);
      const timer = setTimeout(() => setTransitioning(false), 300);
      return () => clearTimeout(timer);
    }
  }, [activePageNumber, currentPageIndex, onPageChange, setTransitioning]);

  // Check if highlight should be visible on current page
  const isHighlightVisible =
    activeBoundingBox !== null &&
    activePageNumber === currentPageIndex + 1 &&
    !isTransitioning;
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-auto bg-gray-100 p-6 flex items-start justify-center">
        <div
          className="bg-white shadow-lg border rounded-sm transition-transform relative"
          style={{
        ...
        >
          ...
          {/* Fax content */}
          <div className="p-6 space-y-4" style={{ fontSize: `${(12 * zoom) / 100}px` }}>
            {currentPage.contentDescription ? (
              /* Realistic document content */
              <pre className="font-mono text-[10px] leading-tight whitespace-pre-wrap text-gray-800 overflow-hidden">
                {currentPage.contentDescription}
              </pre>
            ) : (
              /* Fallback: simulated content for pages without detailed description */
              <>
                {/* Header */}
